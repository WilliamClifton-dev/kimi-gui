...
![Kimi GUI 中文首页](docs/assets/homepage-zh.png)
...
`v0.2.1` 是当前面向中文用户的 MVP，修复了首个 Windows 安装包无法启动的问题。目前已经支持：
...
前往 [GitHub Releases](https://github.com/WilliamClifton-dev/kimi-gui/releases) 下载：

- `Kimi-GUI-Setup-版本-x64.exe`：安装版，适合大多数用户
- `Kimi-GUI-Portable-版本-x64.exe`：免安装版，下载后直接运行

当前安装包尚未购买代码签名证书，Windows 可能显示 SmartScreen 提示。请只从本仓库的 Releases 页面下载，并在确认发布者和文件来源后运行。

使用真实 Kimi 会话前，仍需要在本机安装并登录 Kimi CLI。Kimi GUI 负责把配置、会话、审批和错误信息做成可视化界面，目前还不能替代底层运行时本身。

### 开发者从源码运行

环境要求：Node.js 20 或更高版本、npm，以及 Windows、macOS 或 Linux。
...
| `npm run dist:win` | 生成 Windows 安装版和免安装版 |
